...
class DoctorPatientProfileSerializer(serializers.Serializer):
    ...
    def get_medicalRecords(self, obj):
        doctor_profile = self.context.get("doctor_profile")
        queryset = obj.medical_records.select_related(
            "doctor",
            "appointment",
            "appointment__slot",
...
class DoctorDirectoryPublicSerializer(serializers.ModelSerializer):
    ...
    def get_fullName(self, obj):
        return (
            obj.full_name
            or obj.preferred_name
            or (obj.user.email if obj.user else "Doctor")
        )
    ...
    def get_qualifications(self, obj):
        ...
        return [
            qualification.strip()
            for qualification in obj.qualifications.split(",")
    ...
    def get_nextAvailable(self, obj):
        if (
            ...
            and "available_slots" in obj._prefetched_objects_cache
        ):
            today = timezone.localdate()
            slots = sorted(
            ...
            next_slot = (
                obj.available_slots.filter(date__gte=timezone.localdate())
...
class DoctorDirectoryDetailSerializer(DoctorDirectoryPublicSerializer):
    ...
    class Meta(DoctorDirectoryPublicSerializer.Meta):
        fields = DoctorDirectoryPublicSerializer.Meta.fields + [
            "contactNumber",
            "email",
        ]
