...
class AvailableDoctorsView(APIView):
    ...
    def get(self, request):
        ...
        all_doctors = DoctorProfile.objects.select_related("user").filter(
        ...
        verified_doctor_ids = set(
            DoctorHospitalVerification.objects.filter(
                hospital=hospital, status=DoctorHospitalVerification.Status.VERIFIED
...
class HospitalAdminProfileView(APIView):
    ...
    def get(self, request):
        profile = (
            HospitalAdminProfile.objects.select_related("user")
...
class ActivityLogListView(APIView):
    ...
    def get(self, request):
        ...
        if not hospital_id:
            return Response(
                {"detail": "hospital_id required"}, status=status.HTTP_400_BAD_REQUEST
            )

        # Ensure user is admin for hospital
        if not HospitalAdmin.objects.filter(
            user=request.user, hospital_id=hospital_id, is_active=True
        ).exists():
            return Response(
                {"detail": "Not authorized"}, status=status.HTTP_403_FORBIDDEN
            )

        logs = ActivityLog.objects.filter(hospital_id=hospital_id).order_by(
            "-created_at"
        )[:500]
...
class ReportsView(APIView):
    ...
    def get(self, request):
        ...
        booked_last_30 = Appointment.objects.filter(
        ...
        doctors = (
            ...
            .annotate(
                ...
                patient_count=Count(
                    ...
                    filter=Q(appointments__hospital=admin_role.hospital)
                    & active_appointments,
                    distinct=True,
                ),
            )
...
class HospitalAppointmentListView(APIView):
    ...
    def get(self, request):
        hospital_id = request.query_params.get("hospital_id")
        admin_queryset = HospitalAdmin.objects.filter(
            user=request.user, is_active=True
        ).select_related("hospital")
        if hospital_id:
            admin_queryset = admin_queryset.filter(hospital_id=hospital_id)

        admin_role = admin_queryset.first()
        if not admin_role:
            return Response(
...
class ManageHospitalDoctorView(APIView):
    ...
    def post(self, request):
        ...
        if not doctor_id or action not in ["add", "remove"]:
            return Response(
                {
                    "detail": "Invalid doctor_id or action. Action must be 'add' or 'remove'."
                },
        ...
        if action == "add":
            ...
            if (
                not created
                and verification.status != DoctorHospitalVerification.Status.VERIFIED
            ):
            ...
            return Response(
                {"detail": "Doctor added successfully."}, status=status.HTTP_200_OK
            )
            ...
            return Response(
                {"detail": "Doctor removed successfully."}, status=status.HTTP_200_OK
            )
        ...
        try:
            with transaction.atomic():
                ...
                send_doctor_account_credentials_email(
                    email=user.email,
                    password=password,
                    doctor_name=doctor_profile.preferred_name
                    or doctor_profile.full_name,
                )
        except CredentialEmailDeliveryError:
            logger.exception(
                "doctor_account_creation.email_delivery_failed recipient=%s", email
            )
            return Response(
                {
                    "detail": (
...
class ManageDoctorFeesView(APIView):
    ...
    def get(self, request, doctor_id):
        ...
        if not DoctorHospitalVerification.objects.filter(
            ...
            return Response(
                {"detail": "This doctor is not affiliated with your hospital."},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        for payload_key, model_field in fee_fields.items():
            if payload_key in request.data:
                try:
                    ...
                    return Response(
                        {"detail": f"{payload_key} must be a valid number."},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                if value < 0:
                    return Response(
                        {"detail": f"{payload_key} cannot be negative."},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
